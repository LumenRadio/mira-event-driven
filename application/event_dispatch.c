...
// Function prototypes
// ******************************************************************************
PROCESS(event_dispatch_proc, "React on events from modules, with actions in other modules");

/**
 * Return true for events that require taking action directly.
 */
static bool event_requires_synch_action(process_event_t event);

static void on_manager_event(void* data);
static void on_aggregator_event(void* data);
static void on_sampler_event(void* data);
...
int event_dispatch_post(process_event_t event, process_data_t data)
{
